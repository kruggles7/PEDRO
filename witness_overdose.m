...
load INCOME
load RACE
FILE_IN=fopen('Witness_overdose.txt', 'wt'); 
...
print (gcf, '-dpdf', 'Gender_witness_OD.pdf');
close
...
print (gcf, '-dpdf', [ 'Race_witness_OD.pdf']); 
close
...
print (gcf, '-dpdf', [ 'Class_witness_OD.pdf']); 
close
...
print (gcf, '-dpdf', [ 'Education_witness_OD.pdf']); 
close
...
for j=1:9
    ...
    lower=sprintf('%0.1f',round(lower*10)/10);
    if n>0
        temp=['Number in ' soc_23{j} ' OD: Total ' num2str(n) ', ' num2str(sprintf('%.1f',p)) '%, 95% CI (' num2str(lower) '-' num2str(upper) ') '];  
        fprintf(FILE_IN, '%s\n', temp); 
    end 
    stat_mat(indx_race_noOD,j)=0;
    stat_mat(indx_race_OD,j)=1; 
...
print (gcf, '-dpdf', [ 'Income_witness_OD.pdf']); 
close
...
print (gcf, '-dpdf', 'Homeless_witness_OD.pdf'); 
close 
...
print (gcf, '-dpdf', 'Borough_witness_OD.pdf'); 
close
