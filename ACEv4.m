...
for i=1:numel(demograph)
    ...
    if (i==1) %gender M=1, F=2, transgender=3,4
        ...
        p=anova1(stat_mat); 
        m=nanmean(ACE_mat(indx_male)); 
        s=nanstd(ACE_mat(indx_male)); 
        upper=m+z*s/sqrt(numel(indx_male)); 
        lower=m-z*s/sqrt(numel(indx_male)); 

        fprintf(FILE_IN, '%s\n', ['Mean ACE score for males ' num2str(sprintf('%.1f', m)) ' (' num2str(sprintf('%.1f',s)) '), 95% CI ' num2str(sprintf('%.2f', lower)) '-' num2str(sprintf('%.2f', upper)) ]); 
        m=nanmean(ACE_mat(indx_female)); 
        s=nanstd(ACE_mat(indx_female)); 
        upper=m+z*s/sqrt(numel(indx_female)); 
        lower=m-z*s/sqrt(numel(indx_female)); 
        fprintf(FILE_IN, '%s\n', ['Mean ACE score for females ' num2str(sprintf('%.1f', m)) ' (' num2str(sprintf('%.1f',s)) '), 95% CI ' num2str(sprintf('%.2f', lower)) '-' num2str(sprintf('%.2f', upper)) ]); 
...
data_mat=cell2mat(data_mat); 
indx_n=find(data_mat==0); 
indx_y=find(data_mat>0); 

n=260;
P=numel(indx_n)/n*100;  
fprintf(FILE_IN, '%s\n', 'HCV status'); 
temp=['Number HCV- : ' num2str(numel(indx_n)) ', ' num2str(sprintf('%.1f',P)) '%'];  
fprintf(FILE_IN, '%s\n', temp); 
  
P=numel(indx_y)/n*100; 
temp=['Number of HCV+ : ' num2str(numel(indx_y)) ', ' num2str(sprintf('%.1f',P)) '%'];  
fprintf(FILE_IN, '%s\n', temp); 
  
max_n=max(numel(indx_y), numel(indx_n)); 
stat_mat=nan(max_n, 2);  
stat_mat(1:numel(indx_y),1)=ACE_mat(indx_y); 
stat_mat(1:numel(indx_n),2)=ACE_mat(indx_n); 
p=anova1(stat_mat); 
m=nanmean(ACE_mat(indx_y)); 
s=nanstd(ACE_mat(indx_y)); 
upper=m+z*s/sqrt(numel(indx_y)); 
lower=m-z*s/sqrt(numel(indx_y)); 
fprintf(FILE_IN, '%s\n', ['Mean ACE score for HCV+ ' num2str(sprintf('%.1f', m)) ' (' num2str(sprintf('%.1f',s)) '), 95% CI ' num2str(sprintf('%.2f', lower)) '-' num2str(sprintf('%.2f', upper)) ]); 
m=nanmean(ACE_mat(indx_n)); 
s=nanstd(ACE_mat(indx_n)); 
upper=m+z*s/sqrt(numel(indx_n)); 
lower=m-z*s/sqrt(numel(indx_n)); 
fprintf(FILE_IN, '%s\n', ['Mean ACE score for HCV- ' num2str(sprintf('%.1f', m)) ' (' num2str(sprintf('%.1f',s)) '), 95% CI ' num2str(sprintf('%.2f', lower)) '-' num2str(sprintf('%.2f', upper)) ]); 

fprintf(FILE_IN, '%s\n\n', ['ANOVA1 comparing HCV+ vs. HCV- p = ' num2str(sprintf('%.3f', p)) ]);
...
n=260;
P=numel(indx_OD)/n*100;  
fprintf(FILE_IN, '%s\n', 'HCV status'); 
temp=['Number reporting overdose: ' num2str(numel(indx_OD)) ', ' num2str(sprintf('%.1f',P)) '%'];  
fprintf(FILE_IN, '%s\n', temp); 

max_n=max(numel(indx_OD), numel(indx_noOD)); 
stat_mat=nan(max_n, 2);  
stat_mat(1:numel(indx_OD),1)=ACE_mat(indx_OD); 
stat_mat(1:numel(indx_noOD),2)=ACE_mat(indx_noOD); 
p=anova1(stat_mat); 
m=nanmean(ACE_mat(indx_OD)); 
s=nanstd(ACE_mat(indx_OD)); 
upper=m+z*s/sqrt(numel(indx_OD)); 
lower=m-z*s/sqrt(numel(indx_OD)); 
fprintf(FILE_IN, '%s\n', ['Mean ACE score for OD ' num2str(sprintf('%.1f', m)) ' (' num2str(sprintf('%.1f',s)) '), 95% CI ' num2str(sprintf('%.2f', lower)) '-' num2str(sprintf('%.2f', upper)) ]); 
m=nanmean(ACE_mat(indx_noOD)); 
s=nanstd(ACE_mat(indx_noOD)); 
upper=m+z*s/sqrt(numel(indx_noOD)); 
lower=m-z*s/sqrt(numel(indx_noOD)); 
fprintf(FILE_IN, '%s\n', ['Mean ACE score for no OD ' num2str(sprintf('%.1f', m)) ' (' num2str(sprintf('%.1f',s)) '), 95% CI ' num2str(sprintf('%.2f', lower)) '-' num2str(sprintf('%.2f', upper)) ]); 

fprintf(FILE_IN, '%s\n\n', ['ANOVA1 comparing OD vs. no OD p = ' num2str(sprintf('%.3f', p)) ]);
...
for i=1:numel(questions)
    ...
    fprintf(FILE_IN, '%s\n', ['Mean (SD): ' num2str(sprintf('%.1f', m)) ' (' num2str(sprintf('%.1f', s)) ')']);
    indx_nan=find(isnan(x)==0); 
    n=numel(indx_nan); 
    p=n/numel(x)*100; 
    fprintf(FILE_IN, '%s\n', ['Participants reporting use: ' num2str(sprintf('%.0f', n)) ' (' num2str(sprintf('%.1f', p)) '%)']);
    indx_nan=find(isnan(x)==1); 
    n=numel(indx_nan); 
    p=n/numel(x)*100; 
    fprintf(FILE_IN, '%s\n', ['Participants not reporting use: ' num2str(sprintf('%.0f', n)) ' (' num2str(sprintf('%.1f', p)) '%)']);
    min_=min(data_mat); 
    max_=max(data_mat); 
    fprintf(FILE_IN, '%s\n\n', ['Age range: ' num2str(min_) ' - ' num2str(max_)]); 
end 
...
for i=1:numel(questions)
    ...
    %split ACE score by 0-4 and 5+ for each one 
    ace_1_4=find(ACE_mat<5); 
    ace_5=find(ACE_mat>4); 
    data_1_4=data_mat(ace_1_4); 
    data_5=data_mat(ace_5); 
    m=nanmean(data_1_4); 
    s=nanstd(data_1_4); 
    upper=m+z*s/sqrt(numel(ace_1_4)); 
    lower=m-z*s/sqrt(numel(ace_1_4)); 
    fprintf(FILE_IN, '%s\n', ['Mean (SD) age ACE 0-4: ' num2str(sprintf('%.1f', m)) ' (' num2str(sprintf('%.1f', s)) '), 95% CI ' num2str(sprintf('%.2f', lower)) '-' num2str(sprintf('%.2f', upper)) ]); 
    m=nanmean(data_5); 
    s=nanstd(data_5); 
    upper=m+z*s/sqrt(numel(ace_5)); 
    lower=m-z*s/sqrt(numel(ace_5)); 
    fprintf(FILE_IN, '%s\n', ['Mean (SD) age ACE 5+: ' num2str(sprintf('%.1f', m)) ' (' num2str(sprintf('%.1f', s)) '), 95% CI ' num2str(sprintf('%.2f', lower)) '-' num2str(sprintf('%.2f', upper)) ]); 
    stat_mat=nan(260,2); 
    stat_mat(1:numel(ace_1_4),1)=data_1_4; 
    stat_mat(1:numel(ace_5),2)=data_5; 
    p=anova1(stat_mat); 
    fprintf(FILE_IN, '%s\n\n', ['ANOVA1 comparing ACE 0-4 and ACE 5+ p = ' num2str(sprintf('%.3f', p)) ]);
